...
在 Web 开发中，当处理文件时（创建，上传，下载），经常会遇到二进制数据。另一个典型的应用场景是图像处理。
...
![不同arraybuffer视图](https://gitee.com/Topcvan//js-notes-img/raw/master/%E4%B8%8D%E5%90%8Carraybuffer%E8%A7%86%E5%9B%BE.png)

`ArrayBuffer` 是核心对象，是所有的基础，是原始的二进制数据。

但是，如果要写入值或遍历它，基本上几乎所有操作 —— 都必须使用视图（view），例如：
...
注意，没有名为 `TypedArray` 的构造器，它只是表示 `ArrayBuffer` 上的视图之一的通用总称术语：`Int8Array`，`Uint8Array` 及其他，很快就会有完整列表。
...
1. 如果给定的是 `ArrayBuffer` 参数，则会在其上创建视图。
   ...
   可选参数：可以给定起始位置 `byteOffset`（默认为 0）以及 `length`（默认至 buffer 的末尾），这样视图将仅涵盖 `buffer` 的一部分。
...
注意，尽管有类似 `Int8Array` 这样的名称，但 JavaScript 中并没有像 `int`，或 `int8` 这样的单值类型。这是合乎逻辑的，因为 `Int8Array` 不是这些单值的数组，而是 `ArrayBuffer` 上的视图。

**越界行为**

如果尝试将越界值写入类型化数组会出现什么情况？不会报错。但是多余的位被切除。

例如，尝试将 256 放入 `Uint8Array`。256 的二进制格式是 `100000000`（9 位），但 `Uint8Array` 每个值只有 8 位，因此可用范围为 0 到 255。

对于更大的数字，仅存储最右边的（低位有效）8 位，其余部分被切除：

![越界int8](https://gitee.com/Topcvan//js-notes-img/raw/master/%E8%B6%8A%E7%95%8Cint8.png)

257 的二进制格式是 `100000001`（9 位），最右边的 8 位会被存储，因此数组中会有 `1`：

![越界257](https://gitee.com/Topcvan//js-notes-img/raw/master/%E8%B6%8A%E7%95%8C257.png)
...
- 没有 `splice` —— 无法“删除”一个值，因为类型化数组是缓冲区（buffer）上的视图，并且缓冲区（buffer）是固定的、连续的内存区域。我们所能做的就是分配一个零值。
- 无 `concat` 方法。
...
有了这些方法，就可以复制、混合类型化数组，从现有数组创建新数组等。
...
- 通过 `DataView`，可以使用 `.getUint8(i)` 或 `.getUint16(i)` 之类的方法访问数据，是在调用方法时选择格式，而不是在构造的时候。
...
当将混合格式的数据存储在同一缓冲区（buffer）中时，`DataView` 非常有用。例如，存储一个成对序列（16 位整数，32 位浮点数）时，用 `DataView` 可以轻松访问它们。
...
在大多数情况下，直接对类型化数组进行创建和操作，而将 `ArrayBuffer` 作为“通用标识符（common discriminator）”隐藏起来。可以通过 `.buffer` 来访问它，并在需要时创建另一个视图。
...
`BufferSource` 是最常用的术语之一，因为它的意思是“任何类型的二进制数据” —— `ArrayBuffer` 或其上的视图。

这是一份备忘单：

![bufferSource](https://gitee.com/Topcvan//js-notes-img/raw/master/bufferSource.png)

### 二、`TextEncoder&TextDecoder`
...
`ArrayBuffer` 和视图（view）都是 ECMA 标准的一部分，是 JavaScript 的一部分。`ArrayBuffer`，`Uint8Array` 及其他 `BufferSource` 是“二进制数据”，而 [Blob](https://www.w3.org/TR/FileAPI/#dfn-Blob) 则表示“具有类型的二进制数据”。这样可以方便 `Blob` 用于在浏览器中非常常见的上传/下载操作。`XMLHttpRequest`，`fetch` 等进行 Web 请求的方法可以自然地使用 `Blob`，也可以使用其他类型的二进制数据。

在浏览器中，还有其他更高级的对象，在 [File API](https://www.w3.org/TR/FileAPI/) 中有相关描述。

`Blob` 由一个可选的字符串 `type`（通常是 MIME 类型）和 `blobParts` 组成 —— 一系列其他 `Blob` 对象，字符串和 `BufferSource`。

![Blob组成](https://gitee.com/Topcvan//js-notes-img/raw/master/Blob%E7%BB%84%E6%88%90.png)
...
无法直接在 `Blob` 中更改数据，但可以通过 `slice` 获得 `Blob` 的多个部分，从这些部分创建新的 `Blob` 对象，将它们组成新的 `Blob`，等。
...
比如：通过点击链接，可以下载一个具有动态生成的内容为 `hello world` 的 `Blob` 的文件：
...
下面是类似的代码，此代码可以让用户无需任何 HTML 即可下载动态生成的 `Blob`（也就是通过代码模拟用户点击，从而自动下载）：
...
**因此，如果创建一个 URL，那么即使不再需要该 `Blob` 了，它也会被挂在内存中。**

`URL.revokeObjectURL(url)` 从内部映射中移除引用，因此允许 `Blob` 被删除（如果没有其他引用的话），并释放内存。

在上面最后一个示例中，仅使用一次 `Blob`，来进行即时下载，因此立即调用`URL.revokeObjectURL(link.href)`。

而在前一个带有可点击的 HTML 链接的示例中，不调用 `URL.revokeObjectURL(link.href)`，因为那样会使 `Blob` URL 无效。在调用该方法后，由于映射被删除了，因此该 URL 也就不再起作用了。

#### 3.3 `Blob`转换为`Base64`

`URL.createObjectURL` 的一个替代方法是，将 `Blob` 转换为 base64-编码的字符串。

这种编码将二进制数据表示为一个由 0 到 64 的 ASCII 码组成的字符串，非常安全且“可读“。更重要的是 —— 可以在 “data-url” 中使用此编码。

[“data-url”](https://developer.mozilla.org/zh/docs/Web/http/Data_URIs) 的形式为 `data:[<mediatype>][;base64],<data>`。可以在任何地方使用这种 url，和使用“常规” url 一样。
...
可以创建一个图像（image）的、图像的一部分、或者甚至创建一个页面截图的 `Blob`。这样方便将其上传至其他地方。
...
在下面这个示例中，图像只是被复制了，不过可以在创建 blob 之前，从中裁剪图像，或者在 canvas 上对其进行转换：
...
如果更喜欢 `async/await` 而不是 callback：

```javascript
let blob = await new Promise(resolve => canvasElem.toBlob(resolve, 'image/png'));
```

对于页面截屏，可以使用诸如 https://github.com/niklasvh/html2canvas 之类的库。它所做的只是扫一遍浏览器页面，并将其绘制在 `<canvas>` 上。然后，就可以像上面一样获取一个它的 `Blob`。

#### 3.5 `Blob`转换为`ArrayBuffer`

`Blob` 构造器允许从几乎所有东西创建 blob，包括任何 `BufferSource`。

但是，如果需要执行低级别的操作的话，则可以使用 `FileReader` 从 blob 中获取最低级别的 `ArrayBuffer`：
...
更常见的是第二种，从 `<input type="file">` 或拖放或其他浏览器接口来获取文件。在这种情况下，file 将从操作系统（OS）获得 this 信息。
...
这就是从 `<input type="file">` 中获取 `File` 对象的方式：
...
输入（input）可以选择多个文件，因此 `input.files` 是一个类数组对象。这里只有一个文件，所以只取`input.files[0]`。
...
- `readAsDataURL` —— 当想在 `src` 中使用此数据，并将其用于 `img` 或其他标签时。还有一种用于此的读取文件的替代方案：`URL.createObjectURL(file)`。
