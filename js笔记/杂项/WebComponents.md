...
可以通过描述带有自己的方法、属性和事件等的类来创建自定义 HTML 元素。

在 custom elements （自定义标签）定义完成之后，可以将其和 HTML 的内建标签一同使用。

这是一件好事，因为虽然 HTML 有非常多的标签，但仍然是有穷尽的。如果需要像 `<easy-tabs>`、`<sliding-carousel>`、`<beautiful-upload>`…… 这样的标签，内建标签并不能满足我们。

可以把上述的标签定义为特殊的类，然后使用它们，就好像它们本来就是 HTML 的一部分一样。
...
可以通过创建一个带有几个特殊方法的类来完成这件事。这非常容易实现，只需要添加几个方法就行了，而且这些方法都不是必须的。
...
#### 1.1 例子: “time-formatted”

举个例子，HTML 里面已经有 `<time>` 元素了，用于显示日期／时间。但是这个标签本身并不会对时间进行任何格式化处理。

创建一个可以展示适用于当前浏览器语言的时间格式的 `<time-formatted>` 元素：
...
2. 需要通过 `customElements.define(tag, class)` 来注册这个新元素。
3. 接下来在任何地方都可以使用这个新元素了。
...
原因很简单：在 `constructor` 被调用的时候，还为时过早。虽然这个元素实例已经被创建了，但还没有插入页面。在这个阶段，浏览器还没有处理／创建元素属性：调用 `getAttribute` 将会得到 `null`。所以并不能在那里渲染元素。

而且，如果仔细考虑，这样做对于性能更好 —— 推迟渲染直到真正需要的时候。

在元素被添加到文档的时候，它的 `connectedCallback` 方法会被调用。这个元素不仅仅是被添加为了另一个元素的子元素，同样也成为了页面的一部分。因此可以构建分离的 DOM，创建元素并且让它们为之后的使用准备好。它们只有在插入页面的时候才会真的被渲染。

#### 1.2 监视属性

在目前的 `<time-formatted>` 实现中，在元素渲染以后，后续的属性变化并不会带来任何影响。这对于 HTML 元素来说有点奇怪。通常，当一个属性改变的时候，比如 `a.href`，会预期立即看到变化。我们将会在下面修正这一点。
...
在 HTML 解析器构建 DOM 的时候，会按照先后顺序处理元素，先处理父级元素再处理子元素。例如，如果有 `<outer><inner></inner></outer>`，那么 `<outer>` 元素会首先被创建并接入到 DOM，然后才是 `<inner>`。
...
或者，如果需要子元素，可以使用延迟时间为零的 `setTimeout` 来推迟访问子元素。
...
现在 `alert` 在 `(*)` 行展示了 「John」，因为是在 HTML 解析完成之后，才异步执行了这段程序。可以在这个时候处理必要的子元素并且结束初始化过程。
...
3. 最后，插入一个普通的 `<button>` 标签，但添加 `is="hello-button"` 到这个元素，这样就可以使用 custom element：
...
浏览器在内部使用 DOM/CSS 来绘制它们。这个 DOM 结构一般来说对外部是隐藏的，但可以在开发者工具里面看见它。比如，在 Chrome 里，需要打开「Show user agent shadow DOM」选项。

然后 `<input type="range">` 看起来会像这样：

![img](https://zh.javascript.info/article/shadow-dom/shadow-dom-range@2x.png)

在 `#shadow-root` 下看到的就是被称为「shadow DOM」的东西。

不能使用一般的 JavaScript 调用或者选择器来获取内建 shadow DOM 元素。它们不是常规的子元素，而是一个强大的封装手段。

在上面的例子中，可以看到一个有用的属性 `pseudo`。这是一个因为历史原因而存在的属性，并不在标准中。我们可以使用它来给子元素加上 CSS 样式，像这样：
...
重申一次，`pseudo` 是一个非标准的属性。按照时间顺序来说，浏览器首先实验了使用内部 DOM 结构来实现控件，然后，在一段时间之后，shadow DOM 才被标准化来让开发者们，做类似的事。
...
1. Light tree（光明树） —— 一个常规 DOM 子树，由 HTML 子元素组成。在之前章节看到的所有子树都是「光明的」。
...
![img](https://zh.javascript.info/article/shadow-dom/shadow-dom-say-hello@2x.png)
...
- `「closed」` —— `elem.shadowRoot` 永远是 `null`。
  ...
  只能通过 `attachShadow` 返回的指针来访问 shadow DOM（并且可能隐藏在一个 class 中）。浏览器原生的 shadow tree，比如 `<input type="range">`，是封闭的。没有任何方法可以访问它们。

`attachShadow` 返回的 [shadow root](https://dom.spec.whatwg.org/#shadowroot)，和任何元素一样：可以使用 `innerHTML` 或者 DOM 方法，比如 `append` 来扩展它。

将有 shadow root 的元素叫做「shadow tree host」，可以通过 shadow root 的 `host` 属性访问：
...
内建的 `<template>` 元素用来存储 HTML 模板。浏览器将忽略它的内容，仅检查语法的有效性，但是可以在 JavaScript 中访问和使用它来创建其他元素。

从理论上讲，可以在 HTML 中的任何位置创建不可见元素来储存 HTML 模板。那 `<template>` 元素有什么优势？
...
通常，如果在 `<tr>` 内放置类似 `<div>` 的元素，浏览器会检测到无效的 DOM 结构并对其进行“修复”，然后用 `<table>` 封闭 `<tr>` ，那不是我们想要的。而 `<template>` 则完全保留储存的内容。
...
用 `<template>` 重写上一章的 Shadow DOM 示例：
...
就像浏览器内建的 `<select>` 需要 `<option>` 子项，自定义的 `<custom-tabs>` 可能需要实际的标签内容来起作用。并且一个 `<custom-menu>` 可能需要菜单子项。
...
之后，自定义的组件应该正确地渲染成具有给定标题和项目、处理菜单事件等的漂亮菜单。

如何实现呢？

可以尝试分析元素内容并动态复制重新排列 DOM 节点。这是可能的，但是如果要将元素移动到 Shadow DOM，那么文档的 CSS 样式不能在那里应用，因此文档的视觉样式可能会丢失。看起来还需要做一些事情。

幸运的是不需要开发者去做。Shadow DOM 支持 `<slot>` 元素，由 light DOM 中的内容自动填充。
...
![插槽渲染](https://gitee.com/Topcvan//js-notes-img/raw/master/%E6%8F%92%E6%A7%BD%E6%B8%B2%E6%9F%93.png)
...
shadow DOM 中第一个没有名字的 `<slot>` 是一个默认插槽(随后的未命名插槽将被忽略)。它从 light DOM 中获取没有放置在其他位置的所有节点。
...
![默认插槽](https://gitee.com/Topcvan//js-notes-img/raw/master/%E9%BB%98%E8%AE%A4%E6%8F%92%E6%A7%BD.png)
...
<iframe class="code-result__iframe" data-trusted="1" src="https://zh.js.cx/article/slots-composition/menu/" style="display: block; border: 0px; width: 798px; height: 140px; background: var(--iframeBg);"></iframe>
...
因此无需执行任何操作即可更新渲染。但是如果组件想知道插槽的更改，那么可以用 `slotchange` 事件。
...
注意：2 秒后，如果修改了 `slot="title"` 的内容，则不会发生 `slotchange` 事件。因为没有插槽更改。我们修改了 slotted 元素的内容，这是另一回事。

如果想通过 JavaScript 跟踪 light DOM 的内部修改，也可以使用更通用的机制: `MutationObserver`。

#### 4.6 插槽 API

正如之前所见，JavaScript 会查看真实的 DOM，不展开。但是如果 shadow 树有 `{mode: 'open'}` ，那么可以找出哪个元素被放进一个插槽，反之亦然，哪个插槽分配了给这个元素：
...
当不仅需要显示已插入内容的内容，还需要在 JavaScript 中对其进行跟踪时，这些方法非常有用。
...
例如，我们正在创建 `<custom-dialog>` 元素，并且想使它居中。为此，需要对 `<custom-dialog>` 元素本身设置样式。
...
![host选择宿主](https://gitee.com/Topcvan//js-notes-img/raw/master/host%E9%80%89%E6%8B%A9%E5%AE%BF%E4%B8%BB.png)
...
![host(selector)](https://gitee.com/Topcvan//js-notes-img/raw/master/host(selector).png)

现在附加的居中样式只应用于第一个对话框：`<custom-dialog centered>`。

#### 5.4 :host-context(selector)
...
![插槽内容样式](https://gitee.com/Topcvan//js-notes-img/raw/master/%E6%8F%92%E6%A7%BD%E5%86%85%E5%AE%B9%E6%A0%B7%E5%BC%8F.png)

结果是粗体，但不是红色。

如果想要在组件中设置占槽元素的样式，有两种选择。

首先，可以对 `<slot>` 本身进行样式化，并借助 CSS 继承：
...
![slot样式继承](https://gitee.com/Topcvan//js-notes-img/raw/master/slot%E6%A0%B7%E5%BC%8F%E7%BB%A7%E6%89%BF.png)
...
![slotted(selector)](https://gitee.com/Topcvan//js-notes-img/raw/master/slotted(selector).png)

注意，`::slotted` 选择器不能用于任何插槽中更深层的内容。下面这些选择器是无效的：
...
例如，在 shadow DOM 中，可以使用 `--user-card-field-color` CSS 变量来设置字段的样式，而外部文档可以设置它的值：
...
![css变量](https://gitee.com/Topcvan//js-notes-img/raw/master/css%E5%8F%98%E9%87%8F.png)
...
#### 6.2 冒泡（bubbling）, event.composedPath()
...
如果浏览一下 [UI 事件规范](https://www.w3.org/TR/uievents) 就知道，大部分事件都是 `composed: true`：
...
#### 6.4 自定义事件（Custom events）

当我们发送（dispatch）自定义事件，需要设置 `bubbles` 和 `composed` 属性都为 `true` 以使其冒泡并从组件中冒泡出来。

例如，在 `div#outer` shadow DOM 内部创建 `div#inner` 并在其上触发两个事件。只有 `composed: true` 的那个自定义事件才会让该事件本身冒泡到文档外面：
...
注意，如果是嵌套组件，一个 shadow DOM 可能嵌套到另外一个 shadow DOM 中。在这种情况下合成事件冒泡到所有 shadow DOM 边界。因此，如果一个事件仅用于直接封闭组件，也可以在 shadow host 上发送它并设置 `composed: false`。这样它就不在组件 shadow DOM 中，也不会冒泡到更高级别的 DOM。