...
为了找到一个特定的 cookie，可以以 `;` 作为分隔，将 `document.cookie` 分开，然后找到对应的名字。还可以使用正则表达式或者数组函数来实现。
...
如果运行了上面这段代码，会看到多个 cookie。这是因为 `document.cookie=` 操作不是重写整所有 cookie。它只设置代码中提到的 cookie `user`。
...
- `encodeURIComponent` 编码后的 `name=value` 对，大小不能超过 4KB。因此，不能在一个 cookie 中保存大的东西。
...
- `path=/mypath`
  ...
  通常，应该将 `path` 设置为根目录：`path=/`，以使 cookie 对此网站的所有页面可见。
...
默认情况下，cookie 只有在设置的域下才能被访问到。所以，如果 cookie 设置在 `site.com` 下，在 `other.com` 下就无法获取它。

……但是棘手的是，在子域 `forum.site.com` 下也无法获取它！
...
……但是，如果想要批准像 `forum.site.com` 这样的子域访问 cookie，这是可以做到的。当设置一个在 `site.com` 的 cookie 时，应该将 `domain` 选项显式地设置为根域：`domain=site.com`：
...
出于历史原因，`domain=.site.com`（`site.com` 前面有一个点符号）也以相同的方式工作，允许从子域访问 cookie。这是一个旧的表示法，如果需要支持非常旧的浏览器，则应该使用它。
...
日期必须完全采用 GMT 时区的这种格式。可以使用 `date.toUTCString` 来获取它。例如，将 cookie 设置为 1 天后过期。
...
![xsrf攻击](https://gitee.com/Topcvan//js-notes-img/raw/master/xsrf%E6%94%BB%E5%87%BB.png)
...
但是，实现这种防护需要花费时间：需要确保每个表单都具有 token 字段，并且还必须检查所有请求。
...
如果用户不是来自同一网站，那么设置了 `samesite=strict` 的 cookie 永远不会被发送。
...
可以通过使用两个 cookie 来解决这个问题：一个 cookie 用于“一般识别”，仅用于说 “Hello, John”，另一个带有 `samesite=strict` 的 cookie 用于进行数据更改的操作。这样，从网站外部来的用户会看到欢迎信息，但是支付操作必须是从银行网站启动的，这样第二个 cookie 才能被发送。

- **`samesite=lax`**

一种更轻松的方法，该方法可以防止 XSRF 攻击，并且不会破坏用户体验。
...
**因此，如果仅依靠 `samesite` 提供保护，那么在旧版本的浏览器上将很容易受到攻击。**

但是，可以将 `samesite` 与其他保护措施（例如 XSRF token）一起使用，例如 xsrf token，这样可以多增加一层保护，将来，当旧版本的浏览器淘汰时，可能就可以删除 xsrf token 这种方式了。

#### 1.8 httpOnly

这个选项和 JavaScript 没有关系。
...
注意 cookie 的值是经过编码的，所以 `getCookie` 使用了内建方法 `decodeURIComponent` 函数对其进行解码。
...
注意：当更新或删除一个 cookie 时，应该使用和设置 cookie 时相同的路径和域选项。
...
2. 与 banner 一起，`ads.com` 的远程服务器可能会设置带有 `id=1234` 这样的 cookie 的 `Set-Cookie` header。此类 cookie 源自 `ads.com` 域，并且仅在 `ads.com` 中可见：
   ...
   ![第三方cookie](https://gitee.com/Topcvan//js-notes-img/raw/master/%E7%AC%AC%E4%B8%89%E6%96%B9cookie.png)

3. 下次访问 `ads.com` 网站时，远程服务器获取 cookie `id` 并识别用户：

![识别第三方cookie](https://gitee.com/Topcvan//js-notes-img/raw/master/%E8%AF%86%E5%88%AB%E7%AC%AC%E4%B8%89%E6%96%B9cookie.png)

4. 更为重要的是，当用户从 `site.com` 网站跳转至另一个也带有 banner 的网站 `other.com` 时，`ads.com` 会获得该 cookie，因为它属于 `ads.com`，从而识别用户并在他在网站之间切换时对其进行跟踪：

   ![跟踪第三方cookie](https://gitee.com/Topcvan//js-notes-img/raw/master/%E8%B7%9F%E8%B8%AA%E7%AC%AC%E4%B8%89%E6%96%B9cookie.png)
...
如果加载了一个来自第三方域的脚本，例如 `<script src="https://google-analytics.com/analytics.js">`，并且该脚本使用 `document.cookie` 设置了 cookie，那么此类 cookie 就不是第三方的。
...
GDPR 不仅涉及 cookie，还涉及其他与隐私相关的问题。
...
既然已经有了 cookie。为什么还要其他存储对象呢？

- 与 cookie 不同，Web 存储对象不会随每个请求被发送到服务器。因此，可以保存更多数据。大多数浏览器都允许保存至少 2MB 的数据（或更多），并且具有用于配置数据的设置。
...
然后关闭/重新打开浏览器，或者只是在不同的窗口打开同一页面，然后可以这样获取它：
...
这些方法提供了“按照键获取/设置/删除”的功能。但如何获取所有保存的值或键呢？
...
……因此，需要使用 `hasOwnProperty` 检查来过滤掉原型中的字段：
...
它和`LocalStorage`属性和方法是相同的，但是它有更多的限制：
...
注意，该事件还包含：`event.url` —— 发生数据更新的文档的 url。
...
还可以在基于 promise 的包装器（wrapper），如 https://github.com/jakearchibald/idb 的帮助下使用 `async/await`。这要方便的多，但是包装器并不完美，它并不能替代所有情况下的事件。
...
调用之后会返回 `openRequest` 对象，需要监听该对象上的事件：
...
与服务器端数据库不同，IndexedDB 存在于客户端，数据存储在浏览器中。因此，开发人员无法随时都能访问它。因此，当发布了新版本的应用程序，用户访问我们的网页，可能需要更新该数据库。

如果本地数据库版本低于 `open` 中指定的版本，会触发一个特殊事件 `upgradeneeded`。可以根据需要比较版本并升级数据结构。

当数据库还不存在时（从技术上讲，该版本为 `0`），也会触发 `upgradeneeded` 事件。因此，可以执行初始化。

假设发布了应用程序的第一个版本。

接下来就可以打开版本 `1` 中的 IndexedDB 数据库，并在一个 `upgradeneeded` 的处理程序中执行初始化，如下所示：
...
为了避免这样的错误产生，应当检查 `db.version` 并建议用户重新加载页面。使用正确的 HTTP 缓存头（header）来避免之前缓存的旧代码被加载，这样就永远不会遇到此类问题。
...
还有其他方案。例如，可以在 `db.onversionchange` 中更优雅地关闭连接，关闭连接之前提示用户保存数据。如果 `db.onversionchange` 已完成但旧的连接没有被关闭，新的连接将被立即阻塞，那么可以在新的标签页中要求用户关闭其他标签页以更新数据。

这种更新冲突很少发生，但至少应该有一些对其进行处理的程序，例如 `onblocked` 处理程序，以防程序默默卡死而影响用户体验。
...
键的类型必须为数字、日期、字符串、二进制或数组。它是唯一的标识符，所以可以通过键来搜索/删除/更新值。![indexedDB对象库](https://gitee.com/Topcvan//js-notes-img/raw/master/indexedDB%E5%AF%B9%E8%B1%A1%E5%BA%93.png)

类似于 `localStorage`，向存储区添加值时，可以提供一个键。但当存储对象时，IndexedDB 允许将一个对象属性设置为键，这就更加方便了。或者，可以自动生成键。
...
在上面的示例中，启动了事务并发出了 `add` 请求。但正如前面提到的，一个事务可能有多个相关的请求，这些请求必须全部成功或全部失败。那么如何将事务标记为已完成，并不再请求呢？
...
在上面的示例中，可以在新请求 `(*)` 之前创建一个新的 `db.transaction`。
...
因此，出于报告或其他原因，可以使用 `db.onerror` 处理程序捕获所有错误：
...
可以通过在 `request.onerror` 中使用 `event.stopPropagation()` 来停止冒泡，从而停止 `db.onerror` 事件。
...
对象库按键对值进行内部排序。因此，请求的返回值，是按照键的顺序排列的。

#### 3.7 通过带索引的字段搜索

要根据其他对象字段进行搜索，需要创建一个名为“索引（index）”的附加数据结构。
...
首先，需要创建一个索引。它像对象库一样，必须在 `upgradeneeded` 中创建完成：
...
假设库存里有4本书。下面的图片显示了该索引 `index` 的确切内容：

![按索引搜索](https://gitee.com/Topcvan//js-notes-img/raw/master/%E6%8C%89%E7%B4%A2%E5%BC%95%E6%90%9C%E7%B4%A2.png)
...
还可以使用 `IDBKeyRange` 创建范围，并查找 便宜/贵 的书：
...
在例子中，索引是按照被跟踪对象字段价格 `price` 进行内部排序的。所以进行搜索时，搜索结果也会按照价格排序。
...
无论是否有更多的值匹配光标 —— 调用 `onsuccess`。结果中，可以获得指向下一条记录的光标，或者 `undefined`。

在上面的示例中，光标是为对象库创建的。

也可以在索引上创建一个光标。索引是允许按对象字段进行搜索的。在索引上的光标与在对象存储上的光标完全相同 — 它们通过一次返回一个值来节省内存。

对于索引上的游标，`cursor.key` 是索引键（例如：价格），应该使用 `cursor.primaryKey` 属性作为对象的键：
...
将 `onsuccess/onerror` 添加到每个请求是一项相当麻烦的任务。可以通过使用事件委托（例如，在整个事务上设置处理程序）来简化工作，但是 `async/await` 要方便的多。

在本章，会进一步使用一个轻便的承诺包装器 https://github.com/jakearchibald/idb 。它使用 [promisified](https://zh.javascript.info/promisify) IndexedDB 方法创建全局 `idb` 对象。
...
现在有了可爱的“简单异步代码”和「try…catch」捕获的东西。
